            ...
            <div class="tablet-scroll-x">
                @foreach($activeOrders as $table)
                    ...
                    <button wire:click="selectTable({{ $table->id }})"
                        class="tebo-tab shrink-0 {{ $order->status->value === 'ready' ? 'bg-tebo-green/20 border-tebo-green text-tebo-green' : ($order->status->value === 'served' ? 'bg-tebo-blue/20 border-tebo-blue text-tebo-blue' : 'tebo-tab-inactive') }}">
            ...
            <button wire:click="selectTable({{ $table->id }})"
                ...
                @if($order)
                    ...
                    <div class="text-xs font-medium px-2 py-0.5 rounded-full
                        {{ $orderStatus->value === 'ready' ? 'bg-tebo-green text-tebo-dark' : ($orderStatus->value === 'served' ? 'bg-tebo-blue text-tebo-dark' : 'bg-tebo-amber/30 text-tebo-amber') }}">
